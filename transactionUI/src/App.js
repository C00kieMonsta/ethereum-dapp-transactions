...
var transactionsContract = ETHEREUM_CLIENT
  .eth
  .contract(contractABI)
  .at(contractAddress);

// var sendEvent = transactionsContract.transfer({fromBlock: 0, toBlock:
// 'latest'}); convert strings into bytes
const hexToByte = (hex) => {
...
class App extends Component {
  ...
  _onHandleAddPerson = () => {
    ...
    // && address.length === 42
    if (address.length === 42 && firstName !== "" && lastName !== "") {
      const bytesAddress = address
      transactionsContract.addPerson(bytesAddress, firstName, lastName);
      this.setState({enteredAddress: "", enteredFirstName: "", enteredLastName: ""});
    } else {
